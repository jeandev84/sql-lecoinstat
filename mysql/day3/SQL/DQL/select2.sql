...
SELECT *
FROM Clients
WHERE NomClient LIKE '%a%' AND LENGTH(NomClient) - LENGTH(REPLACE(NomClient, 'a', '')) = 1;
*/
...
SELECT *
FROM Clients
WHERE Nom LIKE "%R_a%"; /* apres la lettre 'R' on a n'importe quel caractère et ensuite 'a' */

-- Avec REGEXP (point)
SELECT *
FROM Clients
WHERE Nom REGEXP "R.A"; /* avec REGEXP '_' equivant a point '.' */
...
-- REGEXP donnera la liste des client contenant "ie", "it", "il", "is"
SELECT *
FROM Clients
WHERE Nom REGEXP "i[etls]";

-- REGEXP donnera la liste des client contenant "ei", "ti", "li", "si"
SELECT *
FROM Clients
WHERE Nom REGEXP "[etls]i";


-- Liste des clients dont le nom contient n'importe quelle lettre de 'a' à 'g' suivie de 'i'
-- [a-g] abcdefg, [a-z] abcdefghijklmnopqrstuvwwxyz
SELECT *
FROM Clients
